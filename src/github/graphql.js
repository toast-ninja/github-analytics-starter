...
const logQueryInfoAndRemainingLimits = ({ org, data, query, elapsedMs }) => {
	...
	log.info(
		chalk`{blue GraphQl query name=${queryName}} cost=${cost}, remaining=${remaining}, limit=${limit}, ` +
			`remainingPercent=${remainingPercent}, resetAt=${resetAt}, nextResetAt=${nextResetAt}, ` +
			`githubOrg=${org}, elapsedMs=${elapsedMs}ms`,
	)

	if (queryName === 'unknown name') {
		log.info(
			`Query that does not have a name: query=${removeAllExtraCharacters(query)}`,
		)
	}
}
...
const logMutationInfo = ({ org, mutation, elapsedMs }) => {
	...
	if (mutationName === 'unknown name') {
		log.info(
			`Query that does not have a name: query=${removeAllExtraCharacters(mutation)}`,
		)
	}
}
...
const graphql = (octokit, org, { operationType }) => async (
	...
	if (operationType === 'query') {
		logQueryInfoAndRemainingLimits({ org, data, query: cleanedUpQuery, elapsedMs })
