...
const throttlingPluging = require('@octokit/plugin-throttling')
const retryPlugin = require('@octokit/plugin-retry')
const Octokit = require('@octokit/rest').plugin([throttlingPluging, retryPlugin])
		...
		throttle: {
			...
			onRateLimit: (retryAfter, options) => {
				...
				if (options.request.retryCount < 2) {
					log.warn(
						`Retrying after ${retryAfter} seconds, retry attempt=${options.request
							.retryCount + 1}`,
					)
...
async function createGithubClient(org, cacheKey, accessToken) {
	...
	const octokit = createOctokit({ org, accessToken: githubAccessToken, cacheKey })
